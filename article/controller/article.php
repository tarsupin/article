...
Content::validateClearance($contentData['status'], (int) $contentData['uni_id']);
...
Feed::$returnURL = "/" . $contentData['url_slug'];
...
<p><a href="' . Content::$returnURL . "?" . Link::prepareData("send-tip-article", $contentData['uni_id']) . '">Tip the Author</a> | <a href="' . Content::shareContent($contentID, "article") . '">Share this Article</a> | <a href="' . Content::chatContent($contentID, "article") . '">Chat this Article</a> | <a href="' . Content::setVote($contentID) . '">Like</a> | <a href="' . Content::flag($contentID) . '">Flag</a>';

// Display the Category Follow option
if($contentData['primary_hashtag'])
{
	echo ' | <a href="' . Feed::follow($contentData['primary_hashtag']) . '">Follow #' . $contentData['primary_hashtag'] . '</a>';
}
